						...
						<form id="register-form" method="post">
							...
							<div class="single-contact-form">
								<div class="contact-box name">
									...
									<span id="email_otp_result" class="form-messege field_error" style="color: green;font-family:'Poppins'"></span>
								</div>
								<span class="field_error" id="email_error"></span>
							</div>
							<div class="single-contact-form">
								<div class="contact-box name">
									<input type="text" name="mobile" id="mobile" placeholder="Your Mobile*" style="width:45%" onblur="formatMobile()">
									...
									<span id="mobile_otp_result" class="form-messege field_error font-robo" style="color: green;font-family:'Poppins'"></span>
...
<input type="hidden" id="is_mobile_verified" value="0" />
<script>
	function alertEmailOTP(num) {
		alert(`OTP sent to your email is ${num}`);
	}

	function alertMobileOTP(num) {
		alert(`OTP sent to your mobile is ${num}`);
	}
			...
			jQuery.ajax({
				...
				success: function(result) {
					if (result["result"] == 'done') {
						...
						alertEmailOTP(result["otp"]);
					} else if (result["result"] == 'email_present') {
			...
			jQuery.ajax({
				...
				success: function(result) {
					if (result == 'done') {
						...
						jQuery('#email_otp_result').html("<span>Email id verified <i></i></span>");
						jQuery('#email_otp_result i').addClass('icon-check icons')
	...
	function formatMobile() {
		...
		if (mobile == '') return;
		const formattedPhoneNumber = mobile.replace(phoneRegex, "($1) $2-$3");
		jQuery('#mobile').val(formattedPhoneNumber);
	}
			...
			jQuery.ajax({
				...
				success: function(result) {
					if (result["result"] == 'done') {
						...
						alertMobileOTP(result["otp"]);
					} else if (result["result"] == 'mobile_present') {
			...
			jQuery.ajax({
				...
				success: function(result) {
					if (result == 'done') {
						...
						jQuery('#mobile_otp_result').html("<span>Mobile number verified <i></i></span>");
						jQuery('#mobile_otp_result i').addClass('icon-check icons')
