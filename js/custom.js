...
function user_register() {
	...
	if (is_email_verified == '0') {
		jQuery('#email_error').html('Please verify email');
		is_error = 'yes';
	}
	if (is_mobile_verified == '0') {
		jQuery('#mobile_error').html('Please verify mobile'); is_error = 'yes';
	}
	if (is_error == '') {
		jQuery.ajax({
		...
		jQuery.ajax({
			...
			success: function (result) {
				...
				if (type == 'update' || type == 'remove') {
					...
					// jQuery('#globalErrorAlert').html('<strong>Success!</strong> Cart updated successfully.');
				}
